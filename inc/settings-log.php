...
function dmeng_options_log_page(){
  ...
  if( isset($_POST['action']) && sanitize_text_field($_POST['action'])=='clear' && wp_verify_nonce( trim($_POST['_wpnonce']), 'check-nonce' ) ) :
	...
	$tab = 'gdreg';
	if( isset($_GET['tab'])){
		if(in_array($_GET['tab'], array('gdreg','credit','exchange','repm','search','vote'))) $tab = $_GET['tab'];
	}
	$dmeng_tabs = array(
		'gdreg' => '报名',
		'credit' => __('积分', 'dmeng'),
...
if($tab=='gdreg'){ 
	...
	$sql_where = "FROM $table_name";
	
	$count =  $wpdb->get_var( "SELECT count(reg_id) ".$sql_where );
...
function exportgdreg(){
	location.href = location.href + '&exportgdreg=1';
}
</script>
	<?php
}
